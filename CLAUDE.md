...
- **`tests/test_streamlit_app.py`** — pure helpers (no Streamlit/model): `load_ct_volume` runs against real in-memory DICOMs; `load_wsi_patches` against a `_FakeSlide` mock. Plus real-asset guards that catch upstream/config drift: `TestMlxVlmContract` (introspects the real mlx-vlm API `run_model`/`load_model` depend on — the **only** guard that catches an mlx-vlm upgrade, since every other test mocks it), `TestThemeConfig` (`.streamlit/config.toml`), `TestHooksConfig` (`.claude/settings.json`), `TestCiWorkflow` (`.github/workflows/ci.yml`), and `TestClaudeMd` (this file — asserts every documented path, `tests/` module, and app-spine symbol stays current, so the doc can't silently drift from the code).
